...
function register_my_custom_post_meta_data(){
	...
	register_meta(
		...
		array(
			...
			'description'       => 'used to track all visits to a single post',
			'single'            => true,
			'show_in_rest'      => true,
		)
	);
...
function rest_api_post_views($r){
	$post_id = (isset($r['id']))? $r['id'] : 0;
	if ($post_id){
		$count = get_post_meta($post_id, 'post_visits', true);
		$count++;
		update_post_meta($post_id, 'post_visits', $count);
		return $count;
	} else {
		return 0;
	}
...
function rest_api_get_views($r){
	$post_id = (isset($r['id']))? $r['id'] : false;
	return ($post_id)? get_post_meta($post_id, 'post_visits', true) : 0;
	die(1);
}
...
add_action('rest_api_init', function(){
	register_rest_route('post_tracker/v1', '/post_views/(?P<id>\d+)', array(
		'methods' => WP_REST_Server::CREATABLE,
		'callback' => 'rest_api_post_views',
		'permission_callback' => 'rest_api_is_logged_in'
	));
	register_rest_route('post_tracker/v1', '/get_views/(?P<id>\d+)', array(
		'methods' => WP_REST_Server::READABLE,
		'callback' => 'rest_api_get_views',
	));
	//like logic
